...
function Library:RenderMouse(State)
	-- TODO: add cursor fade?
	local IconEnabled = InputService.MouseIconEnabled;

	local Cursor = Library.Drawing.new('Triangle');
	Cursor.Thickness = 1;
	Cursor.Filled = true;
	Cursor.Visible = true;

	local CursorOutline = Library.Drawing.new('Triangle');
	CursorOutline.Thickness = 1;
	CursorOutline.Filled = false;
	CursorOutline.Color = Color3.new(0, 0, 0);
	CursorOutline.Visible = true;

	while State.Toggled and ScreenGui.Parent do
	...
	InputService.MouseIconEnabled = IconEnabled;
...
function Library:CreateWindow(...)
	...
	local OuterStroke = Library:Create('UIStroke', {
		Color = Outer.BackgroundColor3;
		Parent = Outer;
		Thickness = 1;
	});
	local OuterCorner = Library:Create('UICorner', {
		CornerRadius = Library.BorderRadius;
		Parent = Outer;
	});
	
	Library:AddToRegistry(OuterCorner, {
	...
	Library:AddToRegistry(InnerUICorner, {
		CornerRadius = 'BorderRadius';
	});
	Library:AddToRegistry(Inner, {
		BackgroundColor3 = 'MainColor';
	...
	function Window:AddTab(Name)
		...
		function Tab:ShowTab()
			...
			Library:SetTransCache(Blocker, "BackgroundTransparency", 0)
		...
		function Tab:HideTab()
			Library:SetTransCache(Blocker, "BackgroundTransparency", 1)
		...
		function Tab:AddTabbox(Info)
			...
			function Tabbox:AddTab(Name)
				...
				function Tab:Resize()
					...
					BoxOuter.Size = UDim2.new(1, 0, 0, 26 + Size);
				end;
	...
	function Library:SetTransCache(Object, Prop, Value)
		local Cache = TransparencyCache[Object];
		if not Cache then
			Cache = {};
			TransparencyCache[Object] = Cache
		end
		
		Cache[Prop] = Value
	end
	function Library:GetTransCache(Object)
		local Cache = TransparencyCache[Object]
		if not Cache then
			Cache = {};
			TransparencyCache[Object] = Cache
	...
	function Library:FadeElements(FadeTime, Toggled)
		for _, Desc in next, Outer:GetDescendants() do
			...
			local Cache = Library:GetTransCache(Desc)
	...
	function Library:PlayOuterFade(FadeTime, Toggled)
	...
	function Library:Toggle()
		...
		ModalElement.Modal = State.Toggled
		
		local FadeTime = Config.MenuFadeTime;
		local Toggled = State.Toggled

		if Toggled then
			if Library.CustomMouse then
				task.spawn(function()
					Library:RenderMouse(State)
				end);
			end
			
			Library:PlayOuterFade(FadeTime, Toggled)
			task.wait(FadeTime);
		end;
		
		Library:FadeElements(FadeTime, Toggled)
		task.wait(FadeTime);
		if not Toggled then
			Library:PlayOuterFade(FadeTime, Toggled)
		end
